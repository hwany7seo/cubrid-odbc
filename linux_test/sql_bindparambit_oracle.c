...
sql_bindparambit_oracle (int case_num, char *dsn)
{
  ...
  pass_count += try_bind_variant (hDbc, 1, "SQL_C_BINARY 0xaa -> SQL_BIT",
				  SQL_C_BINARY, SQL_BIT, &byte_val, 1);
  pass_count += try_bind_variant (hDbc, 2, "SQL_C_BINARY 0xaa -> SQL_BINARY",
				  SQL_C_BINARY, SQL_BINARY, &byte_val, 1);
  pass_count += try_bind_variant (hDbc, 3, "SQL_C_BINARY 0xaa -> SQL_VARBINARY",
				  SQL_C_BINARY, SQL_VARBINARY, &byte_val, 1);
  pass_count += try_bind_variant (hDbc, 4, "SQL_C_BIT 1 -> SQL_BIT",
				  SQL_C_BIT, SQL_BIT, &sql_c_bit_one, 1);
  pass_count += try_bind_variant (hDbc, 5, "SQL_C_CHAR '1' -> SQL_BIT",
				  SQL_C_CHAR, SQL_BIT, &ascii_digit_1, 1);
  pass_count += try_bind_variant (hDbc, 6, "SQL_C_CHAR '1' -> SQL_CHAR",
				  SQL_C_CHAR, SQL_CHAR, &ascii_digit_1, 1);
  pass_count += try_bind_variant (hDbc, 7, "SQL_C_BIT 1 -> SQL_BINARY",
